<template>
  <div class="api-key-page">
    ...
    <a-modal
      ...
      title="创建 API Key"
      :ok-text="newApiKey ? '关闭' : '确定'"
      :cancel-text="newApiKey ? '撤销并关闭' : '取消'"
      @ok="handleModalOk"
      @cancel="handleModalCancel"
      :confirm-loading="creating"
    >
      <!-- 创建表单（仅在未创建时显示） -->
      <a-form v-if="!newApiKey" :model="createForm" :label-col="{ span: 6 }">
...
const handleModalCancel = async () => {
  if (newApiKey.value) {
    try {
      await revokeApiKey({ id: newApiKey.value.id })
      message.warning('已撤销刚创建的 API Key')
      await loadApiKeys()
    } catch (error: any) {
