      ...
      <div class="messages-container" ref="messagesContainer">
        ...
        <div v-for="(msg, index) in messages" :key="index" class="message-wrapper" :class="msg.role">
          ...
          <div v-if="msg.role === 'assistant'" class="role-label assistant-label">
            <span>{{ getAIDisplayName(msg) }}</span>
          </div>
...
interface Message {
  role: string
  content: string
  modelName?: string       // 发送该消息时使用的模型名称
  thinking?: string
  answer?: string
...
// 获取 AI 显示名称（优先使用消息中保存的模型名）
const getAIDisplayName = (msg?: Message) => {
  if (msg?.modelName) {
...
const processStream = async (response: Response) => {
  ...
  let thinkingContent = ''
  let responseModelName = ''  // 从流响应中提取实际使用的模型名
      ...
      for (const line of lines) {
        if (line.startsWith('data:')) {
          ...
          try {
            ...
            // 记录响应的模型名称（第一条数据时的模型名即为实际使用的模型）
            if (streamResponse.model && !responseModelName) {
              responseModelName = streamResponse.model
            }
  ...
  // 保存实际使用的模型名称（优先用流响应中的模型名，其次用当前选中的）
  msg.modelName = responseModelName || selectedModel.value?.modelName || getAIDisplayName()
