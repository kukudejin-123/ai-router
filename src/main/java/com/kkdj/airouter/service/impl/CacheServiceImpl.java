...
public class CacheServiceImpl implements CacheService {
    ...
    public void clearCacheOnStartup() {
        ...
        try {
            // 使用 lazy 扫描方式清理所有以 ai:response: 开头的缓存键
            redissonClient.getKeys().deleteByPattern(CACHE_PREFIX + "*");
            log.info("启动时已清理所有 AI 响应缓存");
        } catch (Exception e) {
            log.warn("启动时清理缓存失败", e);
        }
    }
